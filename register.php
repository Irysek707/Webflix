...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (isset($_POST['terms'])) {
    ...
    if (empty($_POST['contact_number'])) {
      $errors[] = 'Enter your contact_number number.';
    } else {
    ...
    if (empty($_POST['security_question_1'])) {
      $errors[] = 'Enter the answer to the security_question_1 question.';
    } else {
      $sq = mysqli_real_escape_string($link, trim($_POST['security_question_1']));
    }
    if (empty($errors)) {
      $q = "INSERT INTO users
            (first_name, last_name, email, pass, contact_number, dob, country, security_question_1, reg_date)
            VALUES ('$fn','$ln','$e', SHA2('$p',256), '$contact_number', '$dob', '$country','$sq', NOW())";
      $r = @mysqli_query($link, $q);
      if ($r) {
  ...
  <form action="register.php" method="post" class="form2">
    ...
    <div class="row">
      <div class="col">Date of birth
        <input type="date" class="form-control" placeholder="Date of birth" aria-label="Date of birth" name="dob"
          id="dob">
      </div>
    </div>
    <br/>
    ...
    <div class="row">
      <div class="col">
        <input type="password" class="form-control" placeholder="Answer" aria-label="security_question_1"
          name="security_question_1" id="security_question_1">
      </div>
    </div>
    ...
    <br />
    <center>If you already have an account <a href="login.php">Sign in here</a>.</center>
</div>
</form>
</div>